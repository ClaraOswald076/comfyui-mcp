...
| `COMFYUI_CODE_PATH` | `COMFYUI_PATH` | Optional path to the ComfyUI checkout (`main.py`, `.venv`, `custom_nodes`) when code and data live under different roots. Comfy-cli node mutations, `apply_manifest` pip/custom-node fallbacks, workflow-lock pack provenance, and legacy Manager snapshot layouts use this checkout; core updates prefer the checkout observed from the connected local server and use this as their fallback. Model and user-state lookups remain independent and continue to use their component-specific live paths or `COMFYUI_PATH`. |
