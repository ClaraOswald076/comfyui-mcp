...
<sub>
  Header made with the thing this post is about: the still is KREA 2 running locally
  (`krea2_turbo` at 8 steps), animated by MiniMax H3 first-last-frame — the same image at both
  ends, which is what makes it loop. Both on one 4090, about two minutes end to end. It is a nod to
  Sun Yuan and Peng Yu's [*Can't Help Myself*](https://www.guggenheim.org/artwork/34812), a robot
  arm that spends its life sweeping up its own leak, though theirs is a great deal sadder.
</sub>
...
- A reviewer that **says no a lot.** Not a rubber stamp: on one small Windows fix, successive rounds
  turned up six P1 defects — and two of those were introduced by the fixes for the previous round's
