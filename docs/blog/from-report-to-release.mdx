...
- **There is no gate specific to dependency changes.** A lockfile edit is reviewed exactly as
  carefully as a comment fix — which is to say, as carefully as one reviewer happened to be that
  round.
- **Nothing is watching our dependencies for known CVEs.** Writing this post is what surfaced it:
  neither repository has Dependabot, CodeQL or an `npm audit` step, and the scanning described
  above looks for *malicious patterns in our own code*, not for a published advisory against
  something we already depend on. The npm package — the one with the hourly auto-updater and the
  widest blast radius — has the least of this. That gap is being closed, and it should have been
  closed before the pipeline got fast.
...
I don't think this is fully solvable at this scale, and I'd rather state the residual risk than
imply a guarantee I can't back. What is tractable is narrowing it, and writing this section
produced the list:

1. Treat report bodies explicitly as **data, not instruction**, in the fixing agent's context.
2. Require a human on any diff touching **dependencies, lockfiles, CI, or the publish path**.
3. Never let a **truncated diff** count as a reviewed one.
4. Add **Dependabot / CVE tracking** to both repositories, npm side first.

None of those existed when this post was drafted. That is the honest state of it: the pipeline was
built to make fixes fast and correct, and its supply-chain surface got less attention than its
correctness surface. Documenting a system is an underrated way to audit one.
