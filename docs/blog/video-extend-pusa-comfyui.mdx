...
- **Overlap a few frames** between segments and drop duplicates at concat to hide
  the seam.
- `get_system_stats (action:"clear_vram")` isn't needed between hops, but **decode/cache long chains to disk** so
...
real footage in addition to decoding the output. Always `get_system_stats (action:"clear_vram")` before
