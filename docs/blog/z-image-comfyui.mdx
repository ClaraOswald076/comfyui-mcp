...
install_comfyui (action:"apply_manifest") path:"packs/z-image-turbo/manifest.yaml"

# Z-Image Base (trainable, more styles)
install_comfyui (action:"apply_manifest") path:"packs/z-image-base/manifest.yaml"
...
- **OOM.** Drop to the **Q5_K_S** GGUF, lower the resolution, and confirm you've set
  `get_system_stats (action:"clear_vram")` before switching from another model family.
