...
**Dead-man switch (v0.46+).** Idle auto-stop lives in the comfyui-mcp process —
...
when comfyui-mcp itself is gone (or the pod exits). The watchdog needs your
RunPod API key *on the pod* to stop it, so create injects it as a pod env var —
RunPod has no scoped keys, so if you'd rather not have the key on the pod, opt
out with `deadman:false` on `runpod_pod_create` (or `RUNPOD_DEADMAN=0`), or
`DEADMAN_DISABLE=1` as a pod env. Console-deployed pods never carry it, and
custom-template deploys (`RUNPOD_TEMPLATE_ID`) default it **off** — pass
