...
`RUNPOD_DEADMAN_BEAT_GRACE_S`). The watchdog needs your RunPod API key *on the
pod* to stop it, so create injects it as a pod env var — RunPod has no scoped
keys, so if you'd rather not have the key on the pod, opt out with
`deadman:false` on `runpod_pod_create` (or `RUNPOD_DEADMAN=0`), or
`DEADMAN_DISABLE=1` as a pod env. Console-deployed pods never carry it.
