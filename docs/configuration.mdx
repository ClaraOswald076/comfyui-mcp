...
<ParamField path="COMFYUI_CODE_PATH" type="string" default="COMFYUI_PATH">
  Optional code-checkout root for split installs where `main.py`, `.venv`, and `custom_nodes`
  are separate from the data/model/user root. Comfy-cli node mutations, `apply_manifest`
  pip/custom-node fallbacks, workflow-lock pack provenance, and legacy Manager snapshot layouts
  use this checkout. Core updates prefer the checkout observed from the running local server and
  use this as their fallback. Data-facing
  tools remain rooted in their live component paths or `COMFYUI_PATH`. Ignored for local
  filesystem mutation in remote mode.
</ParamField>
