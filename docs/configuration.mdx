...
Tools that require a local install (`restart_comfyui` with `action: "start"` / `install_comfyui (action:"apply_manifest")` / `list_local_models` (`action:"remove"`) / `list_output_images` / etc.)
