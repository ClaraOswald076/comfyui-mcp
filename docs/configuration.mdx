...
The secure `panel_request_secret` path accepts these gateway keys for the built-in `comfyui`
server. They are re-read for each subsequent ComfyUI/Manager request, so a saved credential
takes effect without restarting the orchestrator or calling `panel_reload`. The corresponding
gateway keys also appear as masked slots in the panel credential surface; clearing a slot
revokes that key for subsequent requests without a restart. `panel_request_secret` remains
save-only and rejects blank input. The built-in tool child may still be rebuilt by the existing
secret-change path.
...
For model-download proxy routing, set `COMFYUI_DOWNLOAD_PROXY` to an HTTP/HTTPS proxy. This applies only to model-download requests, including redirect probes, payload sniffing, and segmented ranges. When unset, standard `HTTP_PROXY`/`HTTPS_PROXY`/`NO_PROXY` settings are honored; on Windows, a static WinINet proxy from Internet Settings is used when environment proxy variables are absent. Loopback hosts stay direct, and ComfyUI API requests are never routed through this setting.
