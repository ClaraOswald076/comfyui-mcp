...
server. They are re-read for each subsequent ComfyUI/Manager request, so a saved or revoked
credential takes effect without restarting the orchestrator or calling `panel_reload`. The
built-in tool child may still be rebuilt by the existing secret-change path.
