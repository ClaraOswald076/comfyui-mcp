...
<ParamField path="COMFYUI_CODE_PATH" type="string" default="COMFYUI_PATH">
  `main.py`, `.venv`, `custom_nodes`가 데이터·모델과 다른 루트에 있는 분리 설치의 선택적
  코드 checkout 경로입니다. comfy-cli 변경, 워크플로 lock의 pack 출처, 레거시 Manager
  snapshot이 이 경로를 사용합니다. 코어 업데이트는 연결된 로컬 서버에서 관측한 checkout을
  우선하고 이 경로를 fallback으로 사용합니다. 원격 모드의 로컬 파일 변경에는 사용되지 않습니다.
</ParamField>
