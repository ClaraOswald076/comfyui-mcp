...
The full surface is 151 tools with rich JSON schemas (~200 KB, roughly 50k
...
Audio attachments reach the model on this lane, and **only** where the model
actually reports it can hear. Before sending, the backend asks
`POST /api/show` for the model's capabilities:
...
If the capability probe itself can't run, the audio is sent anyway with an
explicit "I can't confirm this reached the model" note: a guard that fails is
not a verdict.
