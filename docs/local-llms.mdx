...
| **Vision** | Works, degraded. The agent can generate but can't *see* — `view_image` critique loops, `get_workflow (action:"from_image")`, and visual comparisons are unavailable, so it flies blind on its own outputs. |
...
The full surface is 70 tools with rich JSON schemas (~200 KB, roughly 50k
