...
(`list_packs` with `action: "check_runtime"`) and **asks before spending paid API
credits**. See
...
| Tool | Effect |
|---|---|
| `list_packs` (`action: "skill_list"` / `"skill_read"`) | Discover and read bundled model-family + workflow skills |
| `list_packs` (`action: "list"` / `"read_workflow"`) | List one-command installer packs (local-GPU / free) and read a pack's graph |
| `list_packs` (`action: "list_templates"`) | List the connected server's custom-node-contributed workflow templates |
...
| `panel_reload` | Soft-reload the agent + its comfyui tool server (new code/tools) or the panel UI, then resume. Does NOT restart the long-lived orchestrator process behind the `panel_*` tools — that takes a user-run process restart |
