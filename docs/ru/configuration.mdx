...
<ParamField path="COMFYUI_CODE_PATH" type="string" default="COMFYUI_PATH">
  Необязательный корень checkout кода для раздельных установок, где `main.py`,
  `.venv` и `custom_nodes` находятся отдельно от данных и моделей. Этот путь используют
  изменения comfy-cli, происхождение пакетов в lock-файлах и старые snapshots Manager;
  обновление ядра предпочитает checkout, обнаруженный у локального сервера. Для локальных
  изменений файлов в удалённом режиме путь игнорируется.
</ParamField>
