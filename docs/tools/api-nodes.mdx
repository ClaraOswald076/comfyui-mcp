...
<ParamField path="action" type="enum" required>
  Which API-node operation to perform. "list" takes an optional `filter`; "schema" requires `class_type`; "generate" requires `class_type` + `inputs` (optional `disable_random_seed`). Options: `list`, `schema`, `generate`.
</ParamField>
