...
<ParamField path="registry_url" type="string" required>
  Registry worker base URL. Must be the default public registry or an origin the operator allowlisted via COMFYUI_MCP_REGISTRY_URLS (the fetch is server-side — open URLs would be SSRF).
</ParamField>
<ParamField path="app_id" type="string" required>
