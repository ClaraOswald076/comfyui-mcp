...
<ParamField path="type" type="enum" default="output">
  Image directory: output (default), input, or temp
...
<ParamField path="type" type="enum">
  ComfyUI dir for the output ref (default 'output').
  Options: `output`, `input`, `temp`.
</ParamField>
<ParamField path="path" type="string">
...
<ParamField path="format" type="enum">
  Response shape: markdown (default, human/agent-readable) or json (&#123;images:[&#123;filename,subfolder,kind,size,modified&#125;]&#125; — for app clients building pick grids).
