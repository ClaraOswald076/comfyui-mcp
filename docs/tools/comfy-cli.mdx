...
<ParamField path="promptId" type="string">
  Single prompt id. Required for status/watch/cancel; accepted for wait (normalized into a one-element promptIds list).
</ParamField>
<ParamField path="promptIds" type="string[]">
  Prompt ids to wait on. For wait, use this or promptId or all=true.
</ParamField>
