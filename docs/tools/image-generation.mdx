...
<ParamField path="weight_type" type="enum">
  IPAdapter weight mode (default 'standard' — required by current IPAdapter_plus builds)
...
<ParamField path="model_family" type="enum" required>
  Audio model family — determines which workflow template and model loaders to use
...
<ParamField path="timesignature" type="enum">
  TextEncodeAceStepAudio1.5 time signature (ACE only, one of '2'/'3'/'4'/'6', default: '4')
  Options: `2`, `3`, `4`, `6`.
</ParamField>
...
<ParamField path="audio_quality" type="enum">
  SaveAudioMP3 bitrate/quality (ACE only, one of 'V0'/'128k'/'320k', default: '320k')
...
<ParamField path="mode" type="enum" required>
  "text" = text-to-3D from prompt; "image" = image-to-3D from an uploaded input image.
  Options: `text`, `image`.
</ParamField>
