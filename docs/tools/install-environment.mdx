...
<ParamField path="action" type="enum" default="status">
  status: report install mode + current vs latest version + dev-link note (never errors). update: update to the latest published version (refuses on a dev link; no-op when already up to date or for npx). Options: `action:"status"`, `action:"update"`.
</ParamField>
...
<ParamField path="action" type="enum" required>
  Which setting to change. HTTP API: set_preview_method, set_db_mode, set_component_policy, set_update_policy, set_channel, reset_queue. config.ini fallback: set_network_mode, set_security_level. Options: `action:"set_preview_method"`, `action:"set_db_mode"`, `action:"set_component_policy"`, `action:"set_update_policy"`, `action:"set_channel"`, `action:"reset_queue"`, `action:"set_network_mode"`, `action:"set_security_level"`.
</ParamField>
