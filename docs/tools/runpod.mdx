...
<ParamField path="deadman" type="boolean">
  Arm the pod-side dead-man watchdog (default true): the pod STOPS ITSELF if comfyui-mcp's heartbeats stop (process crash/offline — boot grace ~45min, then ~20min without beats). Requires injecting your RunPod API key into the pod's env (unscoped key — any process on the pod could read it); false deploys without the watchdog.
</ParamField>
