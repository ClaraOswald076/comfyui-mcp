...
manifest, and the manifest path for `install_comfyui (action:"apply_manifest")`.
...
<Note>
  ...
  (`install_comfyui (action:"apply_manifest")`) and loading its ready workflow (`panel_load_workflow pack:<name>`)
...
<ParamField path="refresh" type="boolean">
  Bypass the read-through cache and regenerate, overwriting the cached entry.
</ParamField>
...
<ParamField path="graph" type="object | string">
  ...
  (`list_packs` with `action: "check_runtime"`) and ask before spending paid credits.
</ParamField>
