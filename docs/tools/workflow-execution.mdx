...
<Info>19 tools. Generated from the live MCP tool schemas — do not edit by hand; run `npm run docs:gen`.</Info>
...
<ParamField path="template" type="string" required>
  Template name/id: a bundled pack directory name (list_packs) or a custom-node-contributed workflow template name (list_workflow_templates).
</ParamField>
...
<ParamField path="timeout_s" type="number">
  Max seconds to wait when wait:true (default 300). On timeout the job keeps running; poll get_job_status.
</ParamField>
<ParamField path="disable_random_seed" type="boolean">
