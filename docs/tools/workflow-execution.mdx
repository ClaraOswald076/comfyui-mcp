...
<Info>22 tools. Generated from the live MCP tool schemas — do not edit by hand; run `npm run docs:gen`.</Info>
...
<ParamField path="workflows" type="object[]">
  Array of ComfyUI workflows in API format (node ID -&gt; &#123;class_type, inputs&#125;). Mutually exclusive with workflow+sweep.
</ParamField>
<ParamField path="workflow" type="object">
  One base workflow in API format, used with `sweep`.
</ParamField>
<ParamField path="sweep" type="object[]">
  Param sweep: one job per override set, e.g. [&#123;"cfg":6&#125;,&#123;"cfg":8,"steps":30&#125;]. Each key is set on every node that already has that input.
</ParamField>
<ParamField path="disable_random_seed" type="boolean">
  If true, do not randomize seed values (default randomizes per job).
</ParamField>
...
<ParamField path="batch_id" type="string" required>
  The batch_id returned by submit_batch.
</ParamField>
...
<ParamField path="timeout_s" type="number">
  Max seconds to wait (default 300, hard cap 600).
</ParamField>
