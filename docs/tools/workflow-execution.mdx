...
<Tip>**In plain terms:** Everything about runs that have already happened: what a run produced, why one failed, and what settings your past renders actually used.</Tip>
...
**You get back:** That run's outcome and the files it wrote. Omit `prompt_id` to get the most recent run instead of one specific one.
...
**You get back:** A plain reading of what went wrong — a missing model, a bad connection, not enough VRAM — and the suggested fix. Omit `prompt_id` and it looks at the most recent failure. This is the one to reach for when a run fails: it says everything action "list" would, plus what is missing.

**You say:** What sampler settings have actually worked for me on SDXL?
...
**You get back:** A ranked list of sampler/scheduler/steps/CFG combinations you have used before, most-reused first. Read from this machine's own generation log, so it is empty until you have rendered a few things.
