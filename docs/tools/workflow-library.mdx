...
<ParamField path="action" type="enum" required>
  Which write/provenance operation to perform. All three require `filename`; "save" also requires `workflow`. Options: `save`, `lock`, `verify_lock`.
</ParamField>
<ParamField path="filename" type="string">
