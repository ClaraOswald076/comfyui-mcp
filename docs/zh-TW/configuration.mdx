...
<ParamField path="COMFYUI_CODE_PATH" type="string" default="COMFYUI_PATH">
  用於分離式安裝的可選程式碼 checkout 根目錄；此時 `main.py`、`.venv` 與 `custom_nodes`
  和資料及模型分開存放。comfy-cli 變更、工作流程 lock 的節點套件來源，以及舊版
  Manager snapshot 會使用此路徑；核心更新優先採用從已連線本機伺服器觀測到的 checkout，
  並以此路徑作為 fallback。遠端模式不會用它修改本機檔案系統。
</ParamField>
