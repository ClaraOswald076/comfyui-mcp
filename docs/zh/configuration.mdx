...
<ParamField path="COMFYUI_CODE_PATH" type="string" default="COMFYUI_PATH">
  用于分离式安装的可选代码 checkout 根目录；此时 `main.py`、`.venv` 和 `custom_nodes`
  与数据及模型分开存放。comfy-cli 变更、工作流锁的节点包来源以及旧版 Manager 快照
  使用此路径；核心更新优先采用从已连接本地服务器观测到的 checkout，并以此路径作为
  后备。远程模式下不会用它修改本地文件系统。
</ParamField>
