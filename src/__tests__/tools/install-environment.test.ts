...
vi.mock("../../services/manifest.js", () => ({
  applyManifest: (...a: unknown[]) => mocks.applyManifest(...a),
  manifestSchema: z.record(z.string(), z.any()),
}));
...
describe("install_comfyui registration", () => {
  ...
  it("an unknown action returns a clear error result", async () => {
    ...
    expect(text(res)).toMatch(/apply_manifest/);
  });
});
...
describe("install_comfyui actions call the same services with the same arguments", () => {
  ...
  it('action:"apply_manifest" forwards manifest/path as one object, unchanged', async () => {
    mocks.applyManifest.mockResolvedValueOnce({ success: true });
    const res = await call({ action: "apply_manifest", path: "packs/anima/manifest.yaml" });
    expect(mocks.applyManifest).toHaveBeenCalledWith({
      manifest: undefined,
      path: "packs/anima/manifest.yaml",
    });
    expect(JSON.parse(text(res))).toEqual({ success: true });
  });

  // "Exactly one of manifest/path" was never a SCHEMA rule — the service decides,
  // and its error names the real problem. Answering it here would replace a
  // specific message with a generic one.
  it('action:"apply_manifest" with neither argument still reaches the service', async () => {
    mocks.applyManifest.mockRejectedValueOnce(
      new Error("Provide exactly one of `manifest` or `path`"),
    );
    const res = await call({ action: "apply_manifest" });
    expect(mocks.applyManifest).toHaveBeenCalledWith({ manifest: undefined, path: undefined });
    expect(text(res)).toMatch(/exactly one of/i);
  });
});
...
describe("the eight install/environment names are retired", () => {
  const old = [
    "update_comfyui",
    "update_all",
    "install_panel",
