...
function runGitCheckout(baseUrl: string, ref: string, basePath?: string): void {
  // basePath is the CALL-SCOPED local ComfyUI root (install_comfyui (action:"apply_manifest") threads an
 ...
 * bare "python" on PATH. `basePath` is the CALL-SCOPED install root (install_comfyui (action:"apply_manifest")
...
export interface InstallOptions {
   ...
   *  threaded by callers (e.g. install_comfyui (action:"apply_manifest")) that resolve an adopted
...
// Callers rely on that — install_comfyui (action:"apply_manifest") does `installCustomNode(...).then().catch()`
