 ...
 *  We only move on to the next GPU type when RunPod explicitly rejected the
 ...
 *  response was lost) we reconcile by listing pods under the requested name:
 *  if this call's pod appeared, return it; otherwise fail WITHOUT retrying so a
 *  lost-response create can never fan out into extra billable pods. */
export async function createPod(opts: RunpodCreateOptions = {}): Promise<RunpodPod> {
  ...
  for (const gpuTypeId of gpuTypeIds) {
    try {
      ...
      // (retrying other GPU types with the same bad key is pointless).
      ...
      if (isProvablyNotCreatedError(err)) {
        ...
        // GPU type is safe to try.
        attempts.push(`${gpuTypeId}: ${msg}`);
        continue;
      }
      ...
      throw new Error(
        `RunPod pod creation failed on ${gpuTypeId} and it could not be confirmed whether a pod ` +
