...
export function registerBatchTools(server: McpServer): void {
  server.tool(
    ...
    "Run MANY ComfyUI workflows under one durable batch_id. Driven by the `action` parameter:\n" +
      ...
      '- action:"status" — Per-job status for `batch_id`: each prompt_id\'s state (pending/running/done/error/unknown) plus rollup counts and all_terminal. Same status source as get_job_status.\n' +
