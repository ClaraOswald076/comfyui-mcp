 ...
 *
 * `keyword` keeps its TRUTHINESS check here rather than an absence check: an
 * empty keyword matched every line before this consolidation (`"".toLowerCase()`
 * is a substring of everything) and `?? ` would not have changed that, but
 * `if (args.keyword)` is the code that shipped and its "no filter" behaviour for
 * `""` is the behaviour callers have. This is the filter's own semantics, not a
 * requiredness guard.
 */
