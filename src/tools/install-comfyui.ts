...
import { manifestSchema } from "../services/manifest.js";
...
import { applyManifestAction } from "./manifest.js";

/**
 * The eight install/environment tools collapsed into one action-parameterized
 * `install_comfyui` tool (0.50.0 surface consolidation, slice 13).
 *
 ...
 * by "install" and meaningless elsewhere, `manager_setting` by
 * "configure_manager", and the rest are per-action optionals. Every VALUE
 ...
 * falsiness.
 *
 * THREE FOLDED TOOLS ALREADY HAD AN `action` OF THEIR OWN — install_comfyui(action:'panel'),
 * self_update and configure_manager. Two fields cannot share one name in a flat
 ...
 * to change.
 *
 * BLAST RADIUS IS NOT UNIFORM, and the description says so per action: "install"
 * writes a whole new ComfyUI tree, "self_update" mutates THIS npm package,
 * "panel" mutates the sidebar node-pack, "update"/"update_all" move ComfyUI core
 * and every installed node pack respectively. Nothing here is read-only except
 * "environment", panel_action:"status" and self_update_action:"status".
 */
export function registerInstallComfyUITools(server: McpServer): void {
  server.tool(
    ...
    "Install, update and configure the local ComfyUI installation, its sidebar panel, and this MCP server itself. Driven by the `action` parameter:\n" +
      ...
      '- action:"apply_manifest" — Apply a ComfyUI setup manifest from an inline object (`manifest`) or a .json/.yaml/.yml file (`path`). Composes custom-node installs and model downloads, installs pip packages, and reports apt entries as skipped (system packages need manual/root installation). This is what the installer packs in packs/ are consumed by. LOCAL ComfyUI (COMFYUI_PATH set): nodes/models land on the local filesystem and pip installs into the ComfyUI Python env. REMOTE ComfyUI: custom_nodes and models are routed through the ComfyUI-Manager HTTP API (handled on the host), while pip and apt entries are reported as skipped (no remote equivalent). Each item reports applied/skipped/failed independently.',
    {
      action: z
        .enum([
          "install",
        ...
        .describe(
          'Which install/environment operation to perform. "update", "update_all" and "environment" take no other parameters; "install" requires `target_path`; "panel" takes `panel_action` (+ `version`/`reason` for a pin); "self_update" takes `self_update_action`; "configure_manager" requires `manager_setting` (+ `value`); "apply_manifest" takes exactly one of `manifest` or `path`.',
        ),
      target_path: z
        .string()
        .min(1)
      ...
      manifest: manifestSchema
        .optional()
        .describe(
          'action:"apply_manifest" — inline manifest object. Provide exactly one of `manifest` or `path`.',
        ),
      path: z
        .string()
        .optional()
        .describe(
          'action:"apply_manifest" — path to a .json, .yaml, or .yml manifest file. Provide exactly one of `manifest` or `path`.',
        ),
    },
    async (args) => {
      try {
        switch (args.action) {
          ...
          case "apply_manifest":
            // Deliberately NO presence guard: "exactly one of manifest/path" was
            // never enforced by the old schema either — the manifest service
            // resolves the pair and answers with its own error for none/both, and
            // that error names the real problem better than a generic one here.
            return await applyManifestAction({
          ...
          default: {
            ...
            throw new Error(
              `Unknown install_comfyui action "${String(exhaustive)}". Expected one of: install, update, update_all, panel, self_update, environment, configure_manager, apply_manifest.`,
            );
          }
        }
      } catch (err) {
